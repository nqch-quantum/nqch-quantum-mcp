"""Helpers for nqch-quantum-mcp: client construction and data serialisation."""
...
_TOKEN_ENV = "NQCH_QUANTUM_TOKEN"


def get_client() -> Client:
    """Return an authenticated :class:`nqch_quantum.Client`.

    Reads the API token from the ``NQCH_QUANTUM_TOKEN`` environment variable.
    ...
    if not token:
        raise RuntimeError(
            ...
            "Please export your NQCH Quantum Cloud API token before starting the server."
        )
    return nqch_quantum.Client(token=token)

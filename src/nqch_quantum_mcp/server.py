"""FastMCP server exposing NQCH Quantum Cloud quantum circuit submission tools."""
...
mcp = FastMCP("nqch-quantum-cloud")
...
def submit_circuit(
    ...
    """Submit a quantum circuit to NQCH Quantum Cloud and return the job PID.
...
def get_job_status(pid: str) -> str:
    """Refresh and return the current status of an NQCH Quantum Cloud job.
    ...
    if status == "SUCCESS":
        ...
        lines.append("\nJob finished with an error. Check the NQCH Quantum Cloud dashboard for details.")
...
def get_job_result(pid: str, max_samples: int = 20) -> str:
    """Download and return measurement results for a completed NQCH Quantum Cloud job.
...
@mcp.tool()
def list_jobs() -> str:
    """List all NQCH Quantum Cloud jobs associated with your account.
...
@mcp.tool()
def get_quota() -> str:
    """Return disk usage and project quota information for your NQCH Quantum Cloud account.
    ...
    lines = ["## NQCH Quantum Cloud quota\n"]
...
def delete_job(pid: str) -> str:
    """Delete an NQCH Quantum Cloud job by its PID.
...
def run_server() -> None:
    ...
    token = __import__("os").environ.get("NQCH_QUANTUM_TOKEN")
    if not token:
        print(
            "[nqch-quantum-mcp] WARNING: NQCH_QUANTUM_TOKEN is not set. "

...
| Tool | Description |
|------|-------------|
| `submit_circuit` | Submit a quantum circuit (QASM 2.0 or qibo JSON) to NQCH Quantum Cloud; returns a job PID |
...
Set your NQCH Quantum Cloud API token as an environment variable:

```bash
export NQCH_QUANTUM_TOKEN=your_token_here
```
...
```bash
nqch-quantum-mcp serve
```

### Verify authentication

```bash
nqch-quantum-mcp check-auth
```
...
```json
{
  "mcpServers": {
    "nqch-quantum-cloud": {
      "command": "nqch-quantum-mcp",
      "args": ["serve"],
      "env": {
        "NQCH_QUANTUM_TOKEN": "your_token_here"
      }
    }
  }
}

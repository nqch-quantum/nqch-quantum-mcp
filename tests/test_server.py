"""Tests for nqch-quantum-mcp server tools."""
...
from nqch_quantum_mcp.client import circuit_from_input, result_to_markdown
from nqch_quantum_mcp.server import (
    delete_job,
    get_job_result,
    get_job_status,
    get_quota,
...
def _make_mock_client(job=None):
    """Create a mock nqch_quantum.Client."""
    client = MagicMock()
    client.base_url = "https://quantum.nqch.ae"
...
@patch("nqch_quantum_mcp.server.get_client")
class TestSubmitCircuit:
    ...
    def test_auth_error(self, mock_get_client):
        mock_get_client.side_effect = RuntimeError("NQCH_QUANTUM_TOKEN not set")
...
class TestListJobs:
    def test_empty(self, mock_get_client):
        ...
        with patch("nqch_quantum_mcp.server.QiboApiRequest") as mock_req:
